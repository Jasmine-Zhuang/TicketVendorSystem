...
/*        ArrayList<String> aTime = new ArrayList<>(5);
    ...
    @Test(timeout = 50)
    public void TestCancelOneSeat() {
        flight.ReserveOneSeat("1A");
        if (flight.CancelOneSeat("1A")) {
            assertEquals(10, flight.getAvailableSeats());
            assertEquals("1A", flight.getSeatArray().get(0).get(0));
        }
        assertFalse(flight.CancelOneSeat("10A"));
    }
